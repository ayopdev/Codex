...
export const SeoOverview = ({ data }) => {
  ...
  const { title, metaDescription, headings, imageAlt, canonical, robots, sitemap, error, errorMessage } = data;

  return (
    <div className="flex flex-col gap-6">
      {error ? (
        <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50/80 p-4 text-sm text-amber-800 dark:border-amber-500/40 dark:bg-amber-500/10 dark:text-amber-200">
          <FiAlertTriangle className="mt-0.5 text-base" />
          <div className="space-y-1">
            <p className="font-semibold">We couldn't retrieve the full SEO metadata for this URL.</p>
